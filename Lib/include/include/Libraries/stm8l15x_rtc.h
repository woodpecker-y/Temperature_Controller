...
typedef enum
{
  ...
  RTC_FLAG_RECALPF = ((uint16_t)0x0200), /*!<   Recalibration pending Flag, The status flag RECALPF is automatically set to ??when software
                                                writes to the register RTC_CALRL, indicating that the RTC_CALRx registers are blocked.
                                                When the new calibration settings are taken into account, this Flag returns by hardware to ?? */
...
typedef struct
{
  ...
  uint8_t RTC_Month;      /*!< The RTC Calender Month. */
